...
if defined PY python -c "import sys, sysconfig; raise SystemExit(0 if sys.version_info >= (3, 10) and sysconfig.get_config_var('Py_GIL_DISABLED') != 1 else 1)" >nul 2>nul
if defined PY if errorlevel 1 set "PY="

if not defined PY (
  where py >nul 2>nul
  if not errorlevel 1 set "PY=py -3"
)
if defined PY py -3 -c "import sys, sysconfig; raise SystemExit(0 if sys.version_info >= (3, 10) and sysconfig.get_config_var('Py_GIL_DISABLED') != 1 else 1)" >nul 2>nul
if defined PY if errorlevel 1 set "PY="
...
if defined PY python3 -c "import sys, sysconfig; raise SystemExit(0 if sys.version_info >= (3, 10) and sysconfig.get_config_var('Py_GIL_DISABLED') != 1 else 1)" >nul 2>nul
if defined PY if errorlevel 1 set "PY="

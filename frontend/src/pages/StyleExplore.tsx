import { CircleHelp, Compass, FileUp, FolderPlus, Pause, Play, RotateCcw, Save, Square, WandSparkles } from "lucide-react";
...
function WeightParameters({ targetCount, setTargetCount, artistCount, setArtistCount, lower, setLower, upper, setUpper, mode, setMode, leftDispersion, setLeftDispersion, rightDispersion, setRightDispersion, softBalance, setSoftBalance, softBalanceStrength, setSoftBalanceStrength, setHelpEntry }: { targetCount: number; setTargetCount: (value: number) => void; artistCount: number; setArtistCount: (value: number) => void; lower: number; setLower: (value: number) => void; upper: number; setUpper: (value: number) => void; mode: number; setMode: (value: number) => void; leftDispersion: number; setLeftDispersion: (value: number) => void; rightDispersion: number; setRightDispersion: (value: number) => void; softBalance: boolean; setSoftBalance: (value: boolean) => void; softBalanceStrength: number; setSoftBalanceStrength: (value: number) => void; setHelpEntry: (entry: HelpEntry) => void }) {
...
export function StyleExplore() {
  ...
  return <><div className="mx-auto w-full max-w-7xl space-y-5 px-4 py-5">
    <section className="glass flex flex-wrap items-center gap-3 rounded-2xl p-4"><span className="flex h-10 w-10 items-center justify-center rounded-xl text-white" style={{ background: "var(--accent)" }}><Compass size={20} /></span><div className="min-w-32"><h1 className="font-semibold">探索任务</h1><p className="text-xs text-[var(--muted)]">全局选择、创建与控制</p></div><select className={`${inputClass} min-w-56 flex-1`} value={selectedRunId} onChange={(e) => setSelectedRunId(e.target.value)}><option value="">新建任务 / 未选择</option>{runs.map((item) => <option key={item.id} value={item.id}>{item.name} · {statusLabel(item.status)} · {item.done_count}/{item.target_count}</option>)}</select><input className={`${inputClass} min-w-48 flex-1`} value={taskName} onChange={(e) => setTaskName(e.target.value)} placeholder="新任务名称，例如：厚涂水彩画风" />{!run ? <button className={buttonClass} onClick={createRun} disabled={busy}><WandSparkles size={15} />创建基础探索任务</button> : <div className="flex flex-wrap gap-2">{(run.status === "draft" || run.status === "paused") && <button className={buttonClass} onClick={() => controlRun(run.status === "draft" ? "start" : "resume")} disabled={busy}><Play size={15} />{run.status === "draft" ? "开始生成" : "继续生成"}</button>}{run.status === "running" && <button className={ghostButtonClass} onClick={() => controlRun("pause")} disabled={busy}><Pause size={15} />暂停</button>}{!( ["completed", "cancelled"] as string[]).includes(run.status) && <button className={ghostButtonClass} onClick={() => controlRun("cancel")} disabled={busy}><Square size={15} />结束任务</button>}</div>}{run && <span className="w-full text-xs text-[var(--muted)]">当前：{run.name} · {statusLabel(run.status)} · {run.done_count}/{run.target_count}</span>}</section>
    <div className="grid items-stretch gap-5 xl:grid-cols-[340px_minmax(0,1fr)]">
      <aside className="flex">
        <section className="glass flex h-full w-full flex-col rounded-2xl p-4"><h2 className="mb-3 font-semibold">ArtistPool</h2>

...
下载并运行新项目后，在「设置 → 更新与迁移 → 选择旧项目文件夹」选择旧项目目录。迁移工具会复制卡片、图库、Vibe、设置、词典自定义内容、背景图、引擎运行时和插件运行时数据；程序代码、依赖和构建产物不会迁移。同名文件会先备份到 `.migration-backups/`，迁移完成后请先确认新项目数据正常，再删除旧目录。
